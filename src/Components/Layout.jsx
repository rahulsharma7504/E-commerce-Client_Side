...
const Layout = () => {
  ...
  const filterproducts = async () => {
    try {
      const res = await axios.post(`${process.env.REACT_APP_SERVER_URL}/product/filter`, { checked, radio });
      setProduct(res.data);
    } catch (error) {
      console.error('Error fetching products:', error);
    }
  ...
  const nextPage = async () => {
    try {
      const res = await axios.get(`${process.env.REACT_APP_SERVER_URL}/product/pagination?page=${page + 1}`);
